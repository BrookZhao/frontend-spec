...
`encode-lint` 是[前端编码规范工程化](前端编码规范工程化 https://brookzhao.github.io/frontend-spec/)的配套 Lint 工具，可以为项目一键接入规范、一键扫描和修复规范问题，保障项目的编码规范和代码质量。
...
| 规范                                                              | Lint 工具                                                  | npm 包                                                                             |
| ----------------------------------------------------------------- | ---------------------------------------------------------- | ---------------------------------------------------------------------------------- |
| JavaScript 编码规范 <br/> TypeScript 编码规范 <br/> Node 编码规范 | [ESLint](https://eslint.org/)                              | [eslint-config](https://www.npmjs.com/package/@encodefe/eslint-config)             |
| CSS 编码规范                                                      | [stylelint](https://stylelint.io/)                         | [stylelint-config](https://www.npmjs.com/package/@encodefe/stylelint-config)       |
| Git 规范                                                          | [commitlint](https://commitlint.js.org/#/)                 | [commitlint-config](https://www.npmjs.com/package/@encodefe/commitlint-config)     |
| 文档规范                                                          | [markdownlint](https://github.com/DavidAnson/markdownlint) | [markdownlint-config](https://www.npmjs.com/package/@encodefe/markdownlint-config) |
...
安装完成后，可执行 `encode-lint -h` 以验证安装成功。

### 使用

#### `encode-lint init`：一键接入

在项目根目录执行 `encode-lint init`，即可一键接入规范，为项目安装规范 `Lint` 所需的依赖和配置。

具体会做以下事情：

- 安装各种依赖：包括 `Linter` 依赖，如 [ESLint](https://eslint.org/)、[stylelint](https://stylelint.io/)、[commitlint](https://commitlint.js.org/#/)、[markdownlint](https://github.com/DavidAnson/markdownlint) 等；配置依赖，如 [eslint-config](https://www.npmjs.com/package/@encodefe/eslint-config)、[stylelint-config](https://www.npmjs.com/package/@encodefe/stylelint-config)、[commitlint-config](https://www.npmjs.com/package/@encodefe/commitlint-config)、[markdownlint-config](https://www.npmjs.com/package/@encodefe/markdownlint-config) 等
- 写入各种配置文件，包括：
  ...
  - `encode-lint.config.js` encode-lint 包的一些配置，如启用的功能等
- 配置 git commit 卡口：使用 [husky](https://www.npmjs.com/package/husky) 设置代码提交卡口，在 git commit 时会运行 `encode-lint commit-file-scan` 和 `encode-lint commit-msg-scan` 分别对提交文件和提交信息进行规范检查。`encode-lint commit-file-scan` 默认仅对 error 问题卡口，如果你想对 warn 问题也卡口，可以增加 `--strict` 参数以开启严格模式

> 注 1：如果项目已经配置过 ESLint、stylelint 等 Linter，执行 `encode-lint init` 将会提示存在冲突的依赖和配置，并在得到确认后进行覆盖：
>
> 注 2：如果项目的 .vscode/ 目录被 .gitignore 忽略，可以在拉取项目后单独执行 `encode-lint init --vscode` 命令写入 `.vscode/extensions.json` 和 `.vscode/settings.json` 配置文件

#### `encode-lint scan`：一键扫描
...
#### `encode-lint commit-file-scan` 提交文件扫描
...
#### `encode-lint commit-msg-scan` 提交信息扫描
...
- encode-lint.init(config)：将项目一键接入规范，效果等同于 `encode-lint init`
...
如果你的 TS 项目 commit 卡口和 `encode-lint scan` 运行时间很长，可以通过如下在 `.eslintrc.js` 中增加以下配置提升性能：
